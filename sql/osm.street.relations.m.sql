...
select '{"type":"Feature",'||
        '"properties":{'||
                       '"josm":"'||lower(rm.member_type)||rm.member_id||','||string_agg('r'||rtm.relation_id,',' order by rtm.relation_id)||'",'||
                       '"relationtags":"'||string_agg(rtn.k||'|'||replace(rtn.v,'"','\"'),'&' order by rtm.relation_id)||'",'||
                       '"addrhousenumber":"'||coalesce(nt.v,wt.v,rt.v,'')||'"'||
                     '},'||
        '"geometry":'||
       ...
       '},'
from relation_tags rtm
inner join relation_members rm on rm.relation_id=rtm.relation_id and rm.member_role <> 'is_in'
left join relation_tags rtn on rtn.relation_id=rtm.relation_id and rtn.k='name'
...
left join relation_tags rt on rt.relation_id=rm.member_id and rm.member_type='R' and rt.k='addr:housenumber'
where rtm.k='type' and rtm.v in ('street','associatedStreet')
group by rm.member_id,rm.member_type,nt.k,nt.v,wt.k,wt.v,rt.k,rt.v
