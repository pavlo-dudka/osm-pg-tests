...
where rtt.k='type' and rtt.v like '%treet' and rm2.member_id is null
  ...
  and not exists(select * from exc_street_relations_n exc,relation_members rm where exc.street_relation_id_1=rtt.relation_id and exc.street_relation_id_2=rm.relation_id and rm.member_id=w2.id)
  and not exists(select * from exc_street_relations_n exc,relation_members rm where exc.street_relation_id_2=rtt.relation_id and exc.street_relation_id_1=rm.relation_id and rm.member_id=w2.id)
