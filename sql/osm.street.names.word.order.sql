...
select wt.way_id, wt.k, wt.v, 
  trim(coalesce(p.prefix||' ','')||coalesce(t.title||' ','')||coalesce(n.name||' ','')||
	  regexp_replace(replace(regexp_replace(replace(wt.v||' ',coalesce(p.prefix||' ',''),''),coalesce(t.title||' ',''),'', 'i'), coalesce(n.name||' ',''), ''), coalesce(w.reg||' ',''), ''))||
      coalesce(' '||w.type_f,'') as osm_name,
  trim(coalesce(w.type_f||' ','')||coalesce(p.prefix||' ','')||coalesce(t.title||' ','')||coalesce(n.name||' ','')||
	  regexp_replace(replace(regexp_replace(replace(wt.v||' ',coalesce(p.prefix||' ',''),''),coalesce(t.title||' ',''),'', 'i'), coalesce(n.name||' ',''), ''), coalesce(w.reg||' ',''), '')) as osm_alt_name
from way_tags wt
  left join highways h on wt.way_id=h.id
  left join prefixes p on ' '||wt.v||' ' like '% '||p.prefix||' %'
  left join titles t on ' '||wt.v||' ' ilike '% '||t.title||' %'
