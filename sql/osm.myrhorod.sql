drop table streets_myrhorod;
create table streets_myrhorod(osm_name_uk text);
copy streets_myrhorod(osm_name_uk) from 'osm/street_names/myrhorod.csv' csv quote '"';

with t as (
select w.id,wtn.v name_uk
from relations r
inner join relation_tags rt on rt.relation_id=r.id and rt.k='name' and rt.v='Миргород'
