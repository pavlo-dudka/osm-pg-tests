...
left  join way_tags wt2 on wt2.way_id=w.id and (wt2.k in ('amenity','shop','addr:street','amenity') and wt2.v in ('cafe','restaurant','bar','alcohol','convenience','Регенераторна вулиця','school','kindergarten'))
where
 wt2.k is null and (
...
left  join node_tags wt2 on wt2.node_id=w.id and (wt2.k in ('amenity','shop','addr:street','highway') and wt2.v in ('cafe','restaurant','bar','alcohol','convenience','Регенераторна вулиця','milestone'))
where
 wt2.k is null and (
 trim(wt.v)  similar to '[1-9][0-9\-]*[а-яєі]?(/[1-9][0-9]*[а-я]?)?( к[1-9][0-9]*)?' or         --trim it
lower(wt.v)  similar to '[1-9][0-9\-]*[а-яєі]?(/[1-9][0-9]*[а-я]?)?( к[1-9][0-9]*)?' or         --make it lower
