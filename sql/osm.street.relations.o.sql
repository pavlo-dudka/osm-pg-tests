...
t4 as (
  select t3.relation_id,st_difference(t3.geom,r.linestring) diff,coalesce(rt.v,'') as name,coalesce(rp.v,'') as city
  from t3
    inner join relations r on r.id=place_id
    left join relation_tags rt on rt.relation_id=t3.relation_id and rt.k='name'
    left join relation_tags rp on rp.relation_id=t3.place_id and rp.k='name'),
t5 as (
  select *,(select (st_dumppoints(diff)).geom limit 1) geom
  from t4
  where t4.relation_id not in (select relation_id from t4 where st_geometrytype(diff)='ST_Point'))
select '{"type":"Feature",'||
        '"properties":{'||
                       '"josm":"r'||t5.relation_id||'",'||
