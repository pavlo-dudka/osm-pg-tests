...
b as (
  ...
  and not exists(select * from way_tags wt where wt.way_id=wn.way_id and wt.k='highway' and wt.v in ('platform'))
...
from end_nodes b
inner join highways t on t.id<>b.way_id and t.layer=b.layer and st_dwithin(b.geom,t.linestring,0.01) and st_distance_sphere(b.geom,t.linestring) < (case when t.highway_level='service' then 1 else 5 end)
 left join highways h on h.id=b.way_id 
where st_crosses(h.linestring,t.linestring)='f'
