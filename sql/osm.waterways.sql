...
select '{"type":"Feature","properties":{"josm":"w'||w.id||'","name":"'||coalesce(wtn.v,'')||'","level":"'||wtl.v||'","length":"'||round((st_length(w.linestring::geography)/10))/100||'km"},"geometry":'||st_asgeojson(w.linestring, 5)||'},'
from ways w
