...
where
 (position(lower(wt1.v) in lower(wt3.v))>0
  ...
  or levenshtein_less_equal(lower(wt1.v),lower(wt3.v),4)<4
  )
...
select t.id, v1 as oldv, string_agg(distinct v2, '; ' order by v2) as newv, array_agg(distinct road_id) as road_ids
from t
group by 1,2
having not(v1 = any(array_agg(v2))))
select r.name, oldv, newv, string_agg(distinct a.id::text,',' order by a.id::text), array_to_string(array_agg(distinct road_id), ',') as road_ids
from addr_errors a
