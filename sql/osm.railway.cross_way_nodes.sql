...
create table cross_way_nodes_rail as 
with cn as (select node_id, array_agg(way_id) way_ids from way_nodes where exists(select * from railways where id=way_id) group by node_id having count(distinct way_id)>1),
...
create index idx_cross_way_nodes_rail_way_id on cross_way_nodes_rail(way_id_1,way_id_2);